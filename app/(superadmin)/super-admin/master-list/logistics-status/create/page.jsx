  ...
  const validationSchema = Yup.object({
    logistics: Yup.array().of(
      Yup.object({
        name: Yup.string()
          .trim()
          .required("Logistics status is required")
          .max(100, "Logistics status name too long"),
      })
    ),
  });
  ...
  const handleSubmit = async (values, { setSubmitting,}) => {
    ...
    try {
      const res = await fetch(`/api/superadmin/master-list/logistics-status`, {
        ...
        body: JSON.stringify({ logistics: values.logistics, }),
      });
      ...
      toast.success(data.message || "Logistics status saved.");

      router.push('/super-admin/master-list/logistics-status');
    } catch (error) {
      // console.error("Error submitting form:", error);
      toast.error("Failed to save logistics status");
    } finally {
      setSubmitting(false);
    }
  };
                ...
                {({ push, remove }) => (
                  <>
                    ...
                    <div className="space-y-6">
                      {values.logistics.map((_, index) => (
                        <div
                          key={index}
                          className="relative border p-4 rounded-md"
                        >
                          {/* Delete Button */}
                          {values.logistics.length > 1 && (
                            <button
                              type="button"
