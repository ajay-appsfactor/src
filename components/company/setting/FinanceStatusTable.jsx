...
export default function CompanyFinanceStatusTable() {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const res = await fetch(`/api/company/master-list/finance-status/${id}`, {
        method: "PATCH",
  ...
  const handleEditSave = async () => {
    try {
      ...
      const res = await fetch(
        `/api/company/master-list/finance-status/${editData.id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async () => {
    try {
      setIsDeleting(true);
      const res = await fetch(`/api/company/master-list/finance-status/${deleteId}`, {
        method: "DELETE",
      });
    ...
    <section className="w-full max-w-8xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 px-6 py-4">
        <div className="flex items-center gap-4">
          ...
          >
            <Link href="/settings/finance-status/create">
