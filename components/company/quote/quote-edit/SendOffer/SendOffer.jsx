...
const SendOffer = ({ children, type = "Offer Email", data }) => {
  const [open, setOpen] = useState(false);

  // EMAIL
  const [emailTo, setEmailTo] = useState(data.customer_email || "");
  const [subject, setSubject] = useState(
    `${type} for Order ${data.quote_item_id}`
  );
...
//     try {
//       const res = await fetch("/api/company/quotes/send-email", {
//         method: "POST",
    ...
    <div className="flex flex-col gap-2 text-center">
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="max-w-[95vw] sm:!max-w-screen-xl w-full p-4">
          <DialogHeader>
            <DialogTitle>Send {type} </DialogTitle>
          </DialogHeader>
