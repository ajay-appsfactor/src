...
const InvoiceActions = ({ data }) => {
  ...
  const generatePDFDoc = async (download = false) => {
    ...
    doc.setFontSize(11);
    doc.text("Invoice", col1X, sectionY);
    ...
    // ---------------- FINALIZE ----------------
    if (download) {
      doc.save(`Invoice_${data.quote_item_id}.pdf`);
    } else {
  ...
  const [subject, setSubject] = useState(`Invoice for Order ${data.quote_item_id}`);
