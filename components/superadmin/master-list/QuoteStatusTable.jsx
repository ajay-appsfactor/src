...
export default function QuoteStatusTable() {
  ...
  useEffect(() => {
    const fetchPaymentTerms = async () => {
      try {
        setLoading(true);
        const res = await fetch("/api/superadmin/master-list/quote-status");
        const data = await res.json();

        if (!res.ok) {
          throw new Error(data.error || "Failed to fetch quote terms");
        }

        setPaymentTerms(data.data || []);
        setFilteredTerms(data.data || []);
      } catch (error) {
        toast.error(error.message || "Failed to fetch quote terms");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const res = await fetch(`/api/superadmin/master-list/quote-status/${id}`, {
        method: "PATCH",
  ...
  const handleEditSave = async () => {
    try {
      ...
      const res = await fetch(`/api/superadmin/master-list/quote-status/${editData.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = async () => {
    try {
      setIsDeleting(true);
      const res = await fetch(`/api/superadmin/master-list/quote-status/${deleteId}`, {
        method: "DELETE",
      });
      ...
      toast.error(error.message || "Error deleting quote");
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <section className="w-full max-w-8xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 px-6 py-4">
        <div className="flex items-center gap-4">
          <h2 className="text-lg font-semibold text-gray-800">Quote Status</h2>
          <Button
            asChild
            className="px-5 text-sm bg-slate-800 text-white hover:bg-slate-700"
          >
            <Link href="/super-admin/master-list/quote-status/create">
              <Plus className="mr-2 h-4 w-4" />
              Add New Quote Status
            </Link>
          </Button>
        </div>
        ...
        <div className="relative w-full md:w-64 rounded-md bg-white">
          ...
          <Input
            placeholder="Search quote status..."
          ...
          <TableBody>
            {loading ? (
                  ...
                  <TableCell className="text-center">
                    <div className="flex justify-center gap-2">
                      ...
                      <Button
                        ...
                        aria-label="Delete quote status"
                      >
              ...
              <TableRow>
                ...
                >
                  {searchTerm
                    ? "No matching quote status found"
                    : "No quote status available"}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
        ...
        <DialogContent className="bg-white sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm text-gray-500">
              Update the quote status details below
            </DialogDescription>
          </DialogHeader>
