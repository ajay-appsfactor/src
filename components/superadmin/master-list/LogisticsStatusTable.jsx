...
export default function LogisticsStatusTable() {
  ...
  useEffect(() => {
    const fetchPaymentTerms = async () => {
      try {
        setLoading(true);
        const res = await fetch("/api/superadmin/master-list/logistics-status");
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const res = await fetch(`/api/superadmin/master-list/logistics-status/${id}`, {
        method: "PATCH",
  ...
  const handleEditSave = async () => {
    try {
      ...
      const res = await fetch(
        `/api/superadmin/master-list/logistics-status/${editData.id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed to update logistics");

      toast.success(data.message || "Logistics status updated successfully.");
      setPaymentTerms((prev) =>
  ...
  const handleDelete = async () => {
    try {
      setIsDeleting(true);
      const res = await fetch(`/api/superadmin/master-list/logistics-status/${deleteId}`, {
        method: "DELETE",
      });

      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Failed to delete logistics");

      toast.success(data.message || "Logistics status deleted successfully.");
      setPaymentTerms((prev) => prev.filter((term) => term.id !== deleteId));
      setDeleteOpen(false);
    } catch (error) {
      toast.error(error.message || "Error deleting logistics");
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <section className="w-full max-w-8xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 px-6 py-4">
        <div className="flex items-center gap-4">
          ...
          >
            <Link href="/super-admin/master-list/logistics-status/create">
              <Plus className="mr-2 h-4 w-4" />
              Add New Logistics Status
            </Link>
          </Button>
        </div>
        ...
        <div className="relative w-full md:w-64 rounded-md bg-white">
          ...
          <Input
            placeholder="Search logistics status..."
        ...
        <Table className="border rounded-lg">
          <TableHeader className="bg-muted">
            <TableRow>
              ...
              <TableHead>Logistics Status Name</TableHead>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow>
                ...
                >
                  {searchTerm
                    ? "No matching logistics status found"
                    : "No logistics status available"}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
        ...
        <DialogContent className="bg-white sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold">
              Edit Logistics Status
            </DialogTitle>
            <DialogDescription className="text-sm text-gray-500">
              Update the Logistics status details below
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="flex flex-col">
              <Label htmlFor="name" className="mb-2">
                Logistics Status Name
              </Label>
              <Input
                id="name"
                value={editData.name || ""}
